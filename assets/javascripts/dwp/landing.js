(function () {
  ...
  function playThemeSound(theme) {
    try {
      var src = theme === "dark" ? window.__dwSoundOff : window.__dwSoundOn;
      if (src) { var audio = new Audio(src); audio.play(); }
    } catch (e) { /* ignore */ }
  }
  ...
  $$(".dw-theme-toggle").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      style.textContent = "::view-transition-old(root), ::view-transition-new(root) { mix-blend-mode: normal !important; animation: none !important; }";
      ...
      transition.ready.then(function () {
        ...
        var maxRadius = Math.hypot(Math.max(rect.left, right), Math.max(rect.top, bottom));
  ...
  (function initHeroImage() {
    var container = $(".dw-hero__image[data-hero-images]");
    if (!container) return;
    try {
      var images = JSON.parse(container.getAttribute("data-hero-images"));
      if (!images || images.length < 2) return;
      var img = $(".dw-hero__image-img", container);
      if (!img) return;
  ...
  var hamburger = $("#dw-hamburger");
  var navLinks = $("#dw-nav-links");
  ...
  if ("IntersectionObserver" in window) {
    var revealObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          if (entry.target.classList.contains("dw-stagger")) {
    ...
    $$(".dw-anim, .dw-stagger").forEach(function (el) { revealObserver.observe(el); });
  }

  // ═══════════════════════════════════════════════════════════════════
  // 5. MOUSE PARALLAX
  // ═══════════════════════════════════════════════════════════════════
  var heroImage = $(".dw-hero__image-img");
  var orbs = $$(".dw-orb");
  ...
  if ("IntersectionObserver" in window) {
    ...
    var sr = $("#dw-stats-row"); if (sr) statsObs.observe(sr);
  }

  // ═══════════════════════════════════════════════════════════════════
  // 7. VIDEO MODAL
  // ═══════════════════════════════════════════════════════════════════
  var videoModal = $("#dw-video-modal");
  var videoPlayer = $("#dw-video-player");
  ...
  if (videoModal && videoPlayer) {
    ...
    $$(".dw-hero-play").forEach(function (btn) {
      btn.addEventListener("click", function () {
    ...
    var closeBtn = $(".dw-video-modal__close", videoModal);
    if (closeBtn) {
      closeBtn.addEventListener("click", closeVideoModal);
    }

    var backdrop = $(".dw-video-modal__backdrop", videoModal);
    if (backdrop) {
  ...
  // ═══════════════════════════════════════════════════════════════════
  var designerBadge = $("#dw-designer-badge");
  var designerTooltip = $("#dw-designer-tooltip");
